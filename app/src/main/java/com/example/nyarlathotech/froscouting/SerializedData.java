package com.example.nyarlathotech.froscouting;

import com.google.gson.annotations.Expose;
import com.google.gson.annotations.SerializedName;

import retrofit2.http.Field;

public class SerializedData {

    /*
        This class allows for the POST method to send the data over by
        SETTING
        GETTING
        INSTANTIATING
    */
    
    @SerializedName("entry.1955826634")
    @Expose
    private String Scout;
    @SerializedName("entry.932395635")
    @Expose
    private String Team;
    @SerializedName("entry.2079274681")
    @Expose
    private String Position;
    @SerializedName("entry.2035698356")
    @Expose
    private String Match;
    @SerializedName("entry.1990751905")
    @Expose
    private String Obj1;
    @SerializedName("entry.951731743")
    @Expose
    private String Obj2;
    @SerializedName("entry.676493950")
    @Expose
    private String Obj3;
    @SerializedName("entry.56810673")
    @Expose
    private String Obj4;
    @SerializedName("entry.1576806052")
    @Expose
    private String Obj5;
    @SerializedName("entry.1698851026")
    @Expose
    private String Obj6;
    @SerializedName("entry.493595288")
    @Expose
    private String Obj7;
    @SerializedName("entry.1980453348")
    @Expose
    private String Obj8;
    @SerializedName("entry.1969184503")
    @Expose
    private String Obj9;
    @SerializedName("entry.892792854")
    @Expose
    private String Obj10;
    @SerializedName("entry.468850361")
    @Expose
    private String Obj11;
    @SerializedName("entry.934558003")
    @Expose
    private String Obj12;
    @SerializedName("entry.477911469")
    @Expose
    private String Obj13;
    @SerializedName("entry.1134262734")
    @Expose
    private String Obj14;
    @SerializedName("entry.223171706")
    @Expose
    private String Obj15;
    @SerializedName("entry.1470643266")
    @Expose
    private String Obj16;
    @SerializedName("entry.556585741")
    @Expose
    private String Obj17;
    @SerializedName("entry.1607616538")
    @Expose
    private String Obj18;
    @SerializedName("entry.941683632")
    @Expose
    private String Obj19;
    @SerializedName("entry.1633037719")
    @Expose
    private String Obj20;
    @SerializedName("entry.1122372135")
    @Expose
    private String Obj21;
    @SerializedName("entry.1345782814")
    @Expose
    private String Obj22;
    @SerializedName("entry.1666090452")
    @Expose
    private String Obj23;
    @SerializedName("entry.470463983")
    @Expose
    private String Obj24;
    @SerializedName("entry.511003172")
    @Expose
    private String Obj25;
    @SerializedName("entry.1914660102")
    @Expose
    private String Obj26;
    @SerializedName("entry.235433421")
    @Expose
    private String Obj27;
    @SerializedName("entry.764380940")
    @Expose
    private String Obj28;
    @SerializedName("entry.1572325521")
    @Expose
    private String Obj29;
    @SerializedName("entry.808044220")
    @Expose
    private String Obj30;

    public String getScout() {
        return Scout;
    }

    public void setScout(String scout) {
        Scout = scout;
    }

    public String getTeam() {
        return Team;
    }

    public void setTeam(String team) {
        Team = team;
    }

    public String getPosition() {
        return Position;
    }

    public void setPosition(String position) {
        Position = position;
    }

    public String getMatch() {
        return Match;
    }

    public void setMatch(String match) {
        Match = match;
    }

    public String getObj1() {
        return Obj1;
    }

    public void setObj1(String obj1) {
        Obj1 = obj1;
    }

    public String getObj2() {
        return Obj2;
    }

    public void setObj2(String obj2) {
        Obj2 = obj2;
    }

    public String getObj3() {
        return Obj3;
    }

    public void setObj3(String obj3) {
        Obj3 = obj3;
    }

    public String getObj4() {
        return Obj4;
    }

    public void setObj4(String obj4) {
        Obj4 = obj4;
    }

    public String getObj5() {
        return Obj5;
    }

    public void setObj5(String obj5) {
        Obj5 = obj5;
    }

    public String getObj6() {
        return Obj6;
    }

    public void setObj6(String obj6) {
        Obj6 = obj6;
    }

    public String getObj7() {
        return Obj7;
    }

    public void setObj7(String obj7) {
        Obj7 = obj7;
    }

    public String getObj8() {
        return Obj8;
    }

    public void setObj8(String obj8) {
        Obj8 = obj8;
    }

    public String getObj9() {
        return Obj9;
    }

    public void setObj9(String obj9) {
        Obj9 = obj9;
    }

    public String getObj10() {
        return Obj10;
    }

    public void setObj10(String obj10) {
        Obj10 = obj10;
    }

    public String getObj11() {
        return Obj11;
    }

    public void setObj11(String obj11) {
        Obj11 = obj11;
    }

    public String getObj12() {
        return Obj12;
    }

    public void setObj12(String obj12) {
        Obj12 = obj12;
    }

    public String getObj13() {
        return Obj13;
    }

    public void setObj13(String obj13) {
        Obj13 = obj13;
    }

    public String getObj14() {
        return Obj14;
    }

    public void setObj14(String obj14) {
        Obj14 = obj14;
    }

    public String getObj15() {
        return Obj15;
    }

    public void setObj15(String obj15) {
        Obj15 = obj15;
    }

    public String getObj16() {
        return Obj16;
    }

    public void setObj16(String obj16) {
        Obj16 = obj16;
    }

    public String getObj17() {
        return Obj17;
    }

    public void setObj17(String obj17) {
        Obj17 = obj17;
    }

    public String getObj18() {
        return Obj18;
    }

    public void setObj18(String obj18) {
        Obj18 = obj18;
    }

    public String getObj19() {
        return Obj19;
    }

    public void setObj19(String obj19) {
        Obj19 = obj19;
    }

    public String getObj20() {
        return Obj20;
    }

    public void setObj20(String obj20) {
        Obj20 = obj20;
    }

    public String getObj21() {
        return Obj21;
    }

    public void setObj21(String obj21) {
        Obj21 = obj21;
    }

    public String getObj22() {
        return Obj22;
    }

    public void setObj22(String obj22) {
        Obj22 = obj22;
    }

    public String getObj23() {
        return Obj23;
    }

    public void setObj23(String obj23) {
        Obj23 = obj23;
    }

    public String getObj24() {
        return Obj24;
    }

    public void setObj24(String obj24) {
        Obj24 = obj24;
    }

    public String getObj25() {
        return Obj25;
    }

    public void setObj25(String obj25) {
        Obj25 = obj25;
    }

    public String getObj26() {
        return Obj26;
    }

    public void setObj26(String obj26) {
        Obj26 = obj26;
    }

    public String getObj27() {
        return Obj27;
    }

    public void setObj27(String obj27) {
        Obj27 = obj27;
    }

    public String getObj28() {
        return Obj28;
    }

    public void setObj28(String obj28) {
        Obj28 = obj28;
    }

    public String getObj29() {
        return Obj29;
    }

    public void setObj29(String obj29) {
        Obj29 = obj29;
    }

    public String getObj30() {
        return Obj30;
    }

    public void setObj30(String obj30) {
        Obj30 = obj30;
    }
}
